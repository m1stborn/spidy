...
def zip_saved_files(out_file_name, directory):
    ...
    # try:
    #     shutil.move(f"{out_file_name}.zip", "data/")
    #     write_log('SAVE', f'move success dir{os.listdir()}')  # check
    # except Exception as e:
    #     write_log('SAVE', f'List dir{os.listdir()}')
    #     write_log('SAVE', f'Current dir: {os.getcwd()}')
    #     write_log('SAVE', f'Zipped documents to {out_file_name}.zip')
    #     write_log('SAVE', f"Error {e}")
...
def init(config_name=None):
    ...
    if config_name:
        ...
        write_log('INIT', 'Should spidy save the pages it scrapes to the saved folder? (y/n) (Default: Yes):',
                  status='INPUT')
        while True:
            input_ = input()
            if not bool(input_):
